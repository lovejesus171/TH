[Mesh]
   file = '1D.msh'
[]

[Variables]
  # Name of chemical species
  # used
  [./eaq-]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]  
  [./H]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]  
  [./H2]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]
  [./OH]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]
  [./H2O2]
    block = 'Solution'
    order = FIRST
    initial_condition = 0  #[mol/m3]
  [../]
  [./HO2]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]
  [./HO2-]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]  
  [./H+]
   block = 'Solution'
   order = FIRST
   initial_condition = 1E-4 #[mol/m3] at 25 C with 1mol/l of HS- in solution (Na2S)
  [../]
  [./OH-]
   block = 'Solution'
   order =FIRST
   initial_condition = 1E-4 #[mol/m3]
  [../]
  [./O-]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]
  [./O2-]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../] 
  [./O]
   block = 'Solution'
   order = FIRST
   initial_condition = 0 #[mol/m3] at 25 C with 1mol/l of HS- in solution (Na2S)
  [../]
  [./O2]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]
  #unused
  [./H2O]
   block = 'Solution'
   order = FIRST
   initial_condition = 36667 #[mol/m3] at 600 K
  [../]
  [./HS-]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]
  [./SO42-]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]
  [./O3]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]
  [./O3-]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]  
  [./HO3]
    block = 'Solution'
    order = FIRST
    initial_condition = 0 #[mol/m3]
  [../]
  [./T]
    block = 'Solution'
    order = FIRST
    family = LAGRANGE
    initial_condition = 298.15 #[K]
  [../]
  [./pH]
    block = 'Solution'
    order = FIRST
    family = LAGRANGE
    initial_condition = 11.00 #[pH = log(H+)]
  [../]
[]

[Kernels]
# dCi/dt
  # used
  [./deaqm_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = eaq-
  [../]
  [./dH_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = H
  [../]
  [./dH2_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = H2
  [../]
  [./dOH_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = OH
  [../]
  [./dH2O2_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = H2O2
  [../]
  [./dHO2_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = HO2
  [../]  
  [./dHO2m_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = HO2-
  [../]
  [./dHp_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = H+
  [../]
  [./dOHm_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = OH-
  [../]
  [./dOm_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = O-
  [../]  
  [./dO2m_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = O2-
  [../]
  [./dO_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = O
  [../]
  [./dO2_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = O2
  [../]


  # unused
  [./dHS_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = HS-
  [../]
  [./dH2O_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = H2O
  [../]
  [./dpH_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = pH
  [../]
  [./dSO42m_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = SO42-
  [../]
  [./dO3_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = O3
  [../]  
  [./dO3m_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = O3-
  [../]
  [./dHO3_dt]
    block = 'Solution'
    type = TimeDerivative
    variable = HO3
  [../]
 
  
# Radiolysis source terms
  # used
  [./Source_eaqm]
     block = 'Solution'
  type = Source
  variable = eaq-
  Source = 1.106215869 
  [../] 
  [./Source_H]
     block = 'Solution'
  type = Source
  variable = H
  Source = 0.409117026 
  [../] 
  [./Source_H2]
     block = 'Solution'
  type = Source
  variable = H2
  Source = 0.671957573 
  [../]  
  [./Source_OH]
     block = 'Solution'
  type = Source
  variable = OH
  Source = 1.94502005 
  [../]
  [./Source_H2O2]
     block = 'Solution'
  type = Source
  variable = H2O2
  Source = 0.422830446 
  [../]   
  [./Source_HO2]
     block = 'Solution'
  type = Source
  variable = HO2
  Source = 0.0228557 
  [../]
  [./Source_Hp]
     block = 'Solution'
  type = Source
  variable = H+
  Source = 0
  [../]
  [./Source_OHm]
     block = 'Solution'
  type = Source
  variable = OH-
  Source = 0 
  [../]  
  # unused


        
# HeatConduction terms # unused
  [./heat]
    block = 'Solution'
    type = HeatConduction
    variable = T
  [../]
  [./ie]
    block = 'Solution'
    type = HeatConductionTimeDerivative
    variable = T
  [../]
[]

#unit = 1/s
[ChemicalReactions]
 [./Network]
   block = 'Solution'
   species = 'H+ OH- H2O H2O2 SO42- O2 HS- OH H2 H HO2- O2- eaq- HO2 O- O3 O3- HO3'
   track_rates = False

   equation_constants = 'Ea R T_Re CH2O k2 k3 k4 k5 k6'
   equation_values = '20 8.314 298.15 55347 10^-13.999 10^-11.65 10^-11.9 10^-4.57 10^-9.77'
   equation_variables = 'T pH'

# rate coeff unit : dm3 mol / sec
   reactions = '
OH + OH -> H2O2 : {5.5e6*exp(7.9e3*(1/T_Re-1/T))}
OH + eaq- -> OH- : {3.1e7*exp(1.47e4*(1/T_Re-1/T))}
OH + H -> H2O : {9.3e6*exp(8.2e3*(1/T_Re-1/T))}
OH + HO2 -> O2 + H2O : {7.1e6*exp(1.42e4*(1/T_Re-1/T))}
OH + O2- -> O2 + OH- : {1e7*exp(1.76e4*(1/T_Re-1/T))}
OH + H2O2 -> HO2 + H2O : {2.7e4*exp(1.42e4*(1/T_Re-1/T))}
OH + H2 -> H + H2O : {3.4e4*exp(1.92e4*(1/T_Re-1/T))}
OH + OH- -> H2O + O- : {1.2e7*exp(1.42e4*(1/T_Re-1/T))}
OH + HO2- -> HO2 + OH- : {7.5e6*exp(1.42e4*(1/T_Re-1/T))}
OH + O- -> HO2- : {1.8e7*exp(1.42e4*(1/T_Re-1/T))}
O- + H2O -> OH + OH- : {1.7e3*exp(1.42e4*(1/T_Re-1/T))}
eaq- + eaq- + + H2O + H2O -> H2 + OH- + OH- : {5.5e6*exp(1.42e4*(1/T_Re-1/T))}
eaq- + H + H2O -> H2 + OH- : {2.4e7*exp(1.4e4*(1/T_Re-1/T))}
eaq- + O2- + H2O -> HO2- + OH- : {1.3e7*exp(1.42e4*(1/T_Re-1/T))}
eaq- + HO2 -> HO2- : {2e7*exp(1.42e4*(1/T_Re-1/T))}
eaq- + H2O2 -> OH + OH- : {1.2e7*exp(1.56e4*(1/T_Re-1/T))}
eaq- + O2 -> O2- : {1.9e7*exp(1.36e4*(1/T_Re-1/T))}
eaq- + H+ -> H : {2.3e7*exp(1.26e4*(1/T_Re-1/T))}
eaq- + H2O -> H + OH- : {1.9e-2*exp(1.42e4*(1/T_Re-1/T))}
eaq- + HO2- -> O- + OH- : {3.5e6*exp(1.42e4*(1/T_Re-1/T))}
H + H -> H2 : {5.5e6*exp(1.46e4*(1/T_Re-1/T))}
H + O2- -> HO2- : {2.0e7*exp(1.42e4*(1/T_Re-1/T))}
H + HO2 -> H2O2 : {2.0e7*exp(1.42e4*(1/T_Re-1/T))}
H + H2O2 -> OH + H2O : {5.1e6*exp(1.07e4*(1/T_Re-1/T))}
H + O2 -> HO2 : {2.1e7*exp(1.03e4*(1/T_Re-1/T))}
H + OH- -> eaq- + H2O : {2.2e4*exp(3.76e4*(1/T_Re-1/T))}
HO2 + HO2 -> H2O2 + O2 : {8.4e2*exp(2.06e4*(1/T_Re-1/T))}
HO2 + O2- -> HO2- + O2 : {9.6e4*exp(7.6e3*(1/T_Re-1/T))}
HO2 -> H+ + O2- : {8.0e2*exp(7.6e3*(1/T_Re-1/T))}
H+ + O2- -> HO2- : {5.0e7*exp(1.42e4*(1/T_Re-1/T))}
H2O2 + OH- -> HO2- + H2O : {5.0e5*exp(1.88e4*(1/T_Re-1/T))}
HO2- + H2O -> H2O2 + OH- : {5.7e1*exp(2.77e4*(1/T_Re-1/T))}
H2O2 -> H2O + O : {4.0e-11*exp(6.3e4*(1/T_Re-1/T))}
O + O -> O2 : {5.0e6*exp(1.42e4*(1/T_Re-1/T))}
H2O -> H+ + OH- : {2.599e-8*exp(1.26e4*(1/T_Re-1/T))}
H+ + OH- -> H2O : {1.43e8*exp(1.42e4*(1/T_Re-1/T))}
O2- + O2- + H+ -> HO2- + O2 : {1.77e6*exp(8.0e4*(1/T_Re-1/T))}
'
#HS- + H2O2 + H2O2 + H2O2 + H2O2 -> OH- + H+ + H+ + SO42- : {5.5e-4*exp(-51.3e3/R*(1/T_Re-1/T))}
#HS- + H2O2 + H2O2 + H2O2 + H2O2 -> OH- + H+ + H+ + SO42- : {5.5e-4*exp(-51.3e3/R*(1/T_Re-1/T))}
#HS- + O2 -> SO42- + H+ : {3.6*10^(11.78-3000/T)}
#H+ + OH- -> H2O : {1.4E11/1000}
#H2O -> H+ + OH- : {1.4E11*k2/CH2O}
#H2O2 -> HO2- + H+ : {5E10*k3}
#H+ + HO2- -> H2O2 : {5E10/1000}
#H2O2 + OH- -> HO2- + H2O : {1.3E10/1000}
#HO2- + H2O -> H2O2 + OH- : {1.3E10*k2/k3*CH2O/1000}
#eaq- + H2O -> H + OH- : {1.9E1/1000}
#H + OH- -> eaq- + H2O : {2.2E7/1000}
#H -> eaq- + H+ : {2.3E10*k6}
#eaq- + H+ -> H : {2.3E10/1000}
#OH + OH- -> O- + H2O : {1.3E10/1000}
#O- + H2O -> OH + OH- : {1.3E10*k2/k4*CH2O/1000}
#OH -> O- + H+ : {1E11*k4}
#O- + H+ -> OH : {1E11/1000}
#HO2 -> O2- + H+ : {5E10*k5}
#O2- + H+ -> HO2 : {5E10/1000}
#HO2 + OH- -> O2- + H2O : {5E10/1000}
#O2- + H2O -> HO2 + OH- : {5E10*k2/k5*CH2O/1000}
#eaq- + OH -> OH- : {3E10/1000}
#eaq- + H2O2 -> OH + OH- : {1.1E10/1000}
#eaq- + O2- + H2O -> HO2- + OH- : {1.3E10/CH2O/1000}
#eaq- + HO2 -> HO2- : {2E10/1000}
#eaq- + O2 -> O2- : {1.9E10/1000}
#eaq- + eaq- + H2O + H2O -> H2 + OH- + OH- : {5.5E9/CH2O/1000}
#eaq- + H + H2O -> H2 + OH- : {2.5E10/CH2O/1000}
#eaq- + HO2- -> O- + OH- : {3.5E9/1000}
#eaq- + O- + H2O -> OH- + OH- : {2.2E10/CH2O/1000}
#eaq- + O3- + H2O -> O2 + OH- + OH- : {1.6E10/CH2O/1000}
#eaq- + O3 -> O3- : {3.6E10/1000}
#H + H2O -> H2 + OH : {1.1E1/1000}
#H + O- -> OH- : {1E10/1000}
#H + HO2- -> OH + OH- : {9E7/1000}
#H + O3- -> OH- + O2 : {1E10/1000}
#H + H -> H2 : {7.8E9/1000}
#H + OH -> H2O : {7E9/1000}
#H + H2O2 -> OH + H2O : {9E7/1000}
#H + O2 -> HO2 : {2.1E10/1000}
#H + HO2 -> H2O2 : {1.8E10/1000}
#H + O2- -> HO2- : {1.8E10/1000}
#H + O3 -> HO3 : {3.8E10/1000}
#OH + OH -> H2O2 : {3.6E9/1000}
#OH + HO2 -> H2O + O2 : {6E9/1000}
#OH + O2- -> OH- + O2 : {8.2E9/1000}
#OH + H2 -> H + H2O : {4.3E7/1000}
#OH + H2O2 -> HO2 + H2O : {2.7E7/1000}
#OH + O- -> HO2- : {2.5E10/1000}
#OH + HO2- -> HO2 + OH- : {7.5E9/1000}
#OH + O3- -> O3 + OH- : {2.6E9/1000}
#OH + O3- -> O2- + O2- + H+ : {6E9/1000}
#OH + O3 -> HO2 + O2 : {1.1E8/1000}
#HO2 + O2- -> HO2- + O2 : {8E7/1000}
#HO2 + HO2 -> H2O2 + O2 : {7E5/1000}
#HO2 + O- -> O2 + OH- : {6E9/1000}
#HO2 + H2O2 -> OH + O2 + H2O : {5E-1/1000}
#HO2 + HO2- -> OH + O2 + OH- : {5E-1/1000}
#HO2 + O3- -> O2 + O2 + OH- : {6E9/1000}
#HO2 + O3 -> HO3 + O2 : {5E8/1000}
#O2- + O2- + H2O + H2O -> H2O2 + O2 + OH- + OH- : {1E2/(2*CH2O)/1000}
#O2- + O- + H2O -> O2 + OH- + OH- : {6E8/CH2O/1000}
#O2- + H2O2 -> OH + O2 + OH- : {1.3E-1/1000}
#O2- + HO2- -> O- + O2 + OH- : {1.3E-1/1000}
#O2- + O3- + H2O -> O2 + O2 + OH- + OH- : {1E4/CH2O/1000}
#O2- + O3 -> O3- + O2 : {1.5E9/1000}
#O- + O- + H2O -> HO2- + OH- : {1E9/CH2O/1000}
#O- + O2 -> O3- : {3.6E9/1000}
#O- + H2 -> H + OH- : {8E7/1000}
#O- + H2O2 -> O2- + H2O : {5E8/1000}
#O- + HO2- -> O2- + OH- : {4E8/1000}
#O- + O3- -> O2- + O2- : {7E8/1000}
#O- + O3 -> O2- + O2 : {5E9/1000}
#O3- -> O2 + O- : {3.3E3}
#O3- + H+ -> O2 + OH : {9E10/1000}
#HO3 -> O2 + OH : {1.1E5}
 [../]
[]


[Materials]
  [./hcm]
    type = HeatConductionMaterial
    temp = T
    specific_heat  = 75.309 #[J/(mol*K)]
    thermal_conductivity  = 0.6145 #[W/(m*K)]
  [../]
  [./density]
    type = GenericConstantMaterial
    prop_names = density
    prop_values = 997.10 #[kg/m3]
  [../]
[]

[Executioner]
  type = Transient
  start_time = 0 #[s]
  end_time = 100 #[s]
  solve_type = 'PJFNK'
  l_abs_tol = 1e-3 #1e-11 for HS- + H2O2
#  l_tol = 1e-3 #default = 1e-5
  nl_abs_tol = 1e-3 #1e-11 for HS- + H2O2
#  nl_rel_tol = 1e-7  #default = 1e-7
  l_max_its = 100
  nl_max_its = 100
  dtmax = 10 
  [./TimeStepper]
    type = IterationAdaptiveDT
    cutback_factor = 0.99
    dt = 1e-7
    growth_factor = 1.01
  [../]
[]

[Preconditioning]
  [./smp]
    type = SMP
    full = true
  [../]
[]



[Outputs]
  exodus = true
[]
