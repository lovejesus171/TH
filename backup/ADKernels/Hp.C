...
Hp::computeQpResidual()
{
	...
	Real R = 8.314;
        ...
        return -_Num1 * (1 - _f) * _eps * _k1 * pow(_v[_qp], _m) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a1 * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1))
	       -_Num2 * _f * _eps * _k2 * pow(_v[_qp], _m) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a2 * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E2));
}
