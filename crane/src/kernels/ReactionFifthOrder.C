#include "ReactionFifthOrder.h"

// MOOSE includes
#include "MooseVariable.h"

registerMooseObject("CraneApp", ReactionFifthOrder);

template <>
InputParameters
validParams<ReactionFifthOrder>()
{
  ...
  params.addRequiredCoupledVar("z", "The fifth variable that is reacting.");
  ...
  params.addParam<bool>("_z_eq_u", false, "Whether or not z and u are the same variable.");
...
ReactionFifthOrder::ReactionFifthOrder(const InputParameters & parameters)
  : ADKernel(parameters),
    _v(coupledValue("v")),
    _w(coupledValue("w")),
    _x(coupledValue("x")),
    ...
    _z_eq_u(getParam<bool>("_z_eq_u"))
{
}

ADReal
ReactionFifthOrder::computeQpResidual()
{
  return -_test[_i][_qp] * _stoichiometric_coeff * _reaction_coeff[_qp] * _v[_qp] * _w[_qp] *
         _x[_qp] * _y[_qp] * _z[_qp];
}
