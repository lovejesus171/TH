...
ChemFluxBC::validParams()
{
  ...
  params.addRequiredParam<MaterialPropertyName>("Diffusion_coeff", "Put the value of diffusion coefficient");
...
ChemFluxBC::ChemFluxBC(const InputParameters & parameters)
  : ADIntegratedBC(parameters),
   ...
   _diff_coeff(getMaterialProperty<Real>("Diffusion_coeff")),
...
{
     return _test[_i][_qp] * _Num * _diff_coeff[_qp] * _porosity[_qp] * _tortuosity[_qp] * _grad_C1[_qp] * _normals[_qp];
}
