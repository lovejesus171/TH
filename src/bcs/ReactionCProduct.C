...
#include "ReactionCProduct.h"
#include "MooseVariable.h"
#include <algorithm>
#include <limits>

registerMooseObject("corrosionApp", ReactionCProduct);

defineLegacyParams(ReactionCProduct);

InputParameters
ReactionCProduct::validParams()
{
...
ReactionCProduct::ReactionCProduct(const InputParameters & parameters)
...
Real
ReactionCProduct::computeQpResidual()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
...
Real
ReactionCProduct::computeQpJacobian()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
...
Real
ReactionCProduct::computeQpOffDiagJacobian(unsigned int jvar)
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
