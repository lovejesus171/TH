...
#include "ReactionDProduct.h"

registerMooseObject("corrosionApp", ReactionDProduct);

defineLegacyParams(ReactionDProduct);

InputParameters
ReactionDProduct::validParams()
{
...
ReactionDProduct::ReactionDProduct(const InputParameters & parameters)
...
Real
ReactionDProduct::computeQpResidual()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
...
Real
ReactionDProduct::computeQpJacobian()
{
        return 0;
}
...
Real
ReactionDProduct::computeQpOffDiagJacobian(unsigned int jvar)
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
