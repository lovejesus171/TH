...
{
   return -_Num * _test[_i][_qp] * _eps * (_kF * _C1[_qp] * _C1[_qp] * exp(-_a * _F  / (_R * _T[_qp]) * (_E[_qp] - _EA)) - _kB * _u[_qp]); 
}
...
CuCl2m::computeQpOffDiagJacobian(unsigned int jvar)
{
   ...
   if (jvar == _C_id)
     return -_Num * _test[_i][_qp] * _eps * _kF * 2.0 * _C1[_qp] * _phi[_j][_qp] * exp(-_a * _F /(_R * _T[_qp]) * (_E[_qp] - _EA));
   else if (jvar == _E_id)
