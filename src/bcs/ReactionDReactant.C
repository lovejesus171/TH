...
#include "ReactionDReactant.h"

registerMooseObject("corrosionApp", ReactionDReactant);

defineLegacyParams(ReactionDReactant);

InputParameters
ReactionDReactant::validParams()
{
...
ReactionDReactant::ReactionDReactant(const InputParameters & parameters)
...
Real
ReactionDReactant::computeQpResidual()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
...
Real
ReactionDReactant::computeQpJacobian()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
...
Real
ReactionDReactant::computeQpOffDiagJacobian(unsigned int jvar)
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
