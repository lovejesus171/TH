...
registerMooseObject("corrosionApp", ES2);

defineLegacyParams(ES2);
...
ES2::ES2(const InputParameters & parameters)
...
Real
ES2::computeQpResidual()
{
   return -_Num * _test[_i][_qp] * _eps * _kS * _u[_qp] * _u[_qp] * exp(( 1.0 + _aS) * _F /(_R * _T[_qp]) * _E[_qp]) * exp(-_F / (_R * _T[_qp]) * (_ES12 + _aS3 * _ES3 ));   
}

Real
ES2::computeQpJacobian()
{
   return -_Num * _test[_i][_qp] * _eps * _kS * 2 * _phi[_j][_qp] * _u[_qp] * exp(( 1.0 + _aS) * _F /(_R * _T[_qp]) * _E[_qp]) * exp(-_F / (_R * _T[_qp]) * (_ES12 + _aS3 * _ES3 ));
}

Real
ES2::computeQpOffDiagJacobian(unsigned int jvar)
{
   Real Front;
   Front = -_Num * _test[_i][_qp] * _eps * _kS * _u[_qp] * _u[_qp];
   ...
   if (jvar == _E_id)
     return Front * (1.0 + _aS)/(_R * _T[_qp]) * _F * _phi[_j][_qp] * ExFactor;
   else if (jvar == _T_id)
     return Front * Factor / (_T[_qp] * _phi[_j][_qp]) * ExFactor * (1 - (1 + _aS) * _E[_qp]);
   else
     return 0.0;
}
