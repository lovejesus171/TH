...
#include "ReactionHProduct.h"

registerMooseObject("corrosionApp", ReactionHProduct);

defineLegacyParams(ReactionHProduct);

InputParameters
ReactionHProduct::validParams()
{
...
ReactionHProduct::ReactionHProduct(const InputParameters & parameters)
...
Real
ReactionHProduct::computeQpResidual()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
     
        return -_test[_i][_qp] * _Num *  _f[_qp] * _eps[_qp] * _k1[_qp] * _v[_qp] * exp(_DelH[_qp]/R * (1/Tref - 1/_T[_qp])) * exp(_a1[_qp] * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1[_qp]));
}
Real
ReactionHProduct::computeQpJacobian()
{
        return 0;
}
...
Real
ReactionHProduct::computeQpOffDiagJacobian(unsigned int jvar)
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
        ...
        if (jvar == _T_id)
          return -_test[_i][_qp] * _Num *  _f[_qp] * _eps[_qp] * _k1[_qp] * _v[_qp] * exp(_DelH[_qp]/R * (1/Tref - 1/_T[_qp])) * exp(_a1[_qp] * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1[_qp])) * (_DelH[_qp] - _a1[_qp] * F * (_Ecorr[_qp] - _E1[_qp])) / (R * _T[_qp] * _T[_qp]) * _phi[_j][_qp];
	else if (jvar == _v_id)
          return -_test[_i][_qp] * _Num *  _f[_qp] * _eps[_qp] * _k1[_qp] * _phi[_j][_qp] * exp(_DelH[_qp]/R * (1/Tref - 1/_T[_qp])) * exp(_a1[_qp] * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1[_qp]));
	else
	       return 0;
