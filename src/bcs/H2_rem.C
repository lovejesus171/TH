...
registerMooseObject("corrosionApp", H2_rem);

defineLegacyParams(H2_rem);
...
H2_rem::H2_rem(const InputParameters & parameters)
...
Real
H2_rem::computeQpResidual()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
     
       return -_test[_i][_qp] * _Num1 * _f * _eps * _k1 * pow(_u[_qp], _m) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a1 * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1))
;
}
Real
H2_rem::computeQpJacobian()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
     
       return -_test[_i][_qp] * _Num1 *  _f * _eps * _k1 * _m * _phi[_j][_qp] * pow(_u[_qp], _m-1) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a1 * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1))
;
}


Real
H2_rem::computeQpOffDiagJacobian(unsigned int jvar)
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
