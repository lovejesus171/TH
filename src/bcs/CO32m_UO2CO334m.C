...
#include "CO32m_UO2CO334m.h"

registerMooseObject("corrosionApp", CO32m_UO2CO334m);

defineLegacyParams(CO32m_UO2CO334m);

InputParameters
CO32m_UO2CO334m::validParams()
{
...
CO32m_UO2CO334m::CO32m_UO2CO334m(const InputParameters & parameters)
...
Real
CO32m_UO2CO334m::computeQpResidual()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
...
Real
CO32m_UO2CO334m::computeQpJacobian()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;

          return -_Num1 * (1 - _f) * _eps * _k1 * _m * _phi[_j][_qp] * pow(_u[_qp], _m-1) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a1 * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1))
;

}

Real
CO32m_UO2CO334m::computeQpOffDiagJacobian(unsigned int jvar)
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;

	return -_Num1 * (1 - _f) * _eps * _k1 * pow(_u[_qp], _m) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a1 * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1)) * (_DelH - _a1 * F * (_Ecorr[_qp] - _E1)) / (R * _T[_qp] * _T[_qp]) * _phi[_j][_qp];
;
