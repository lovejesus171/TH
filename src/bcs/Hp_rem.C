...
registerMooseObject("corrosionApp", Hp_rem);

defineLegacyParams(Hp_rem);
...
Hp_rem::Hp_rem(const InputParameters & parameters)
...
Real
Hp_rem::computeQpResidual()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
     
        return -_test[_i][_qp] * _Num1 *  _f * _eps * _k1 * pow(_v[_qp], _m) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a1 * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1));
}
Real
Hp_rem::computeQpJacobian()
{
        return 0;
}
...
Real
Hp_rem::computeQpOffDiagJacobian(unsigned int jvar)
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
        ...
        if (jvar == _T_id)
          return -_test[_i][_qp] * _Num1 *  _f * _eps * _k1 * pow(_v[_qp], _m) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a1 * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1)) * (_DelH - _a1 * F * (_Ecorr[_qp] - _E1)) / (R * _T[_qp] * _T[_qp]) * _phi[_j][_qp];
	else if (jvar == _v_id)
          return -_test[_i][_qp] * _Num1 *  _f * _eps * _k1 * _m * _phi[_j][_qp] * pow(_v[_qp], _m-1) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a1 * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1));
	else
	       return 0;
