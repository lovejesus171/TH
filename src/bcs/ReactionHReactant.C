...
#include "ReactionHReactant.h"

registerMooseObject("corrosionApp", ReactionHReactant);

defineLegacyParams(ReactionHReactant);

InputParameters
ReactionHReactant::validParams()
{
...
ReactionHReactant::ReactionHReactant(const InputParameters & parameters)
...
Real
ReactionHReactant::computeQpResidual()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
     
       return -_test[_i][_qp] * _Num * _f[_qp] * _eps[_qp] * _k1[_qp] * _u[_qp] * exp(_DelH[_qp]/R * (1/Tref - 1/_T[_qp])) * exp(_a1[_qp] * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1[_qp]))
;
}
Real
ReactionHReactant::computeQpJacobian()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
...
Real
ReactionHReactant::computeQpOffDiagJacobian(unsigned int jvar)
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
