...
#include "UO2CO334m_BC.h"
#include "MooseVariable.h"
#include <algorithm>
#include <limits>

registerMooseObject("corrosionApp", UO2CO334m_BC);

defineLegacyParams(UO2CO334m_BC);

InputParameters
UO2CO334m_BC::validParams()
{
...
UO2CO334m_BC::UO2CO334m_BC(const InputParameters & parameters)
...
Real
UO2CO334m_BC::computeQpResidual()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
     
          return -_Num * _f * _eps * _k * _C[_qp] * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E));
}

Real
UO2CO334m_BC::computeQpJacobian()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
...
Real
UO2CO334m_BC::computeQpOffDiagJacobian(unsigned int jvar)
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
	...
	if (jvar == _C_id)
               return -_Num * _f * _eps * _k * 0.66 * _phi[_j][_qp] * pow(_C[_qp], -0.34) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E));
	else if	(jvar == _T_id)
               return -_Num * _f * _eps * _k * pow(_C[_qp], 0.66)  * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E)) * (_DelH - _a * F * (_Ecorr[_qp] - _E)) / (R * _T[_qp] * _T[_qp]) * _phi[_j][_qp];
	else
	       return 0;
