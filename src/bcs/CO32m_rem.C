...
registerMooseObject("corrosionApp", CO32m_rem);

defineLegacyParams(CO32m_rem);

InputParameters
CO32m_rem::validParams()
{
...
CO32m_rem::CO32m_rem(const InputParameters & parameters)
...
Real
CO32m_rem::computeQpResidual()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
     
 		return -_test[_i][_qp] * _Num1 * (1 - _f) * _eps * _k1 * pow(_u[_qp], 0.66) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a1 * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1));
}

Real
CO32m_rem::computeQpJacobian()
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;

          return -_test[_i][_qp] * _Num1 * (1 - _f) * _eps * _k1 * 0.66 * _phi[_j][_qp] * pow(_u[_qp], -0.34) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a1 * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1));
}

Real
CO32m_rem::computeQpOffDiagJacobian(unsigned int jvar)
{
	Real Tref = 298.15;
	Real F = 96485;
	Real R = 8.314;
	...
	if (jvar == _T_id)
		return -_test[_i][_qp] * _Num1 * (1 - _f) * _eps * _k1 * pow(_u[_qp], 0.66) * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_a1 * F /(R * _T[_qp]) * (_Ecorr[_qp] - _E1)) * (_DelH - _a1 * F * (_Ecorr[_qp] - _E1)) / (R * _T[_qp] * _T[_qp]) * _phi[_j][_qp];
