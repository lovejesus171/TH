...
#include "HandUO2_Debug.h"

registerMooseObject("corrosionApp", HandUO2_Debug);

InputParameters
HandUO2_Debug::validParams()
{
...
HandUO2_Debug::HandUO2_Debug(const InputParameters & parameters)
  : Material(parameters),
    _T(coupledValue("T")),
    _C1(coupledValue("C1")),
    _C2(coupledValue("C2")),
...
void
HandUO2_Debug::initQpStatefulProperties()
{
  _IAA[_qp] = 0;
  _IBB[_qp] = 0;
  _ICC[_qp] = 0;
...
void
HandUO2_Debug::computeQpProperties()
{
  Real F = 96485;
  Real R = 8.314;
  Real Tref = 298.15;
  ...
  //Calculate Corrosion Potential at given time and each quadruture point
          while (true)
                {
  // Calculcate current of each reaction
	  ...
	  _IKK[_qp] = - _f * _nK * F * _eps * _kK * _C4[_qp] * exp(_DelH/R * (1/Tref - 1/_T[_qp])) * exp(_aK * F /(R * _T[_qp]) * (_Ecorr[_qp] - _EK));
		...
		_Isum[_qp] = 
	_IAA[_qp] + _IBB[_qp] + _ICC[_qp] + _IDD[_qp] + _IEE[_qp] + _IFF[_qp] + _IGG[_qp] + _IHH[_qp] + _III[_qp] + _IJJ[_qp] + _IKK[_qp];
