...
#include "NoInitUO2Potential.h"

registerMooseObject("corrosionApp", NoInitUO2Potential);

InputParameters
NoInitUO2Potential::validParams()
{
...
NoInitUO2Potential::NoInitUO2Potential(const InputParameters & parameters)
  : Material(parameters),
    _T(coupledValue("T")),
    _C1(coupledValue("C1")),
    _C2(coupledValue("C2")),
...
void
NoInitUO2Potential::initQpStatefulProperties()
{
   _Ecorr[_qp] = _Ecorr_old[_qp];
}

void
NoInitUO2Potential::computeQpProperties()
{
  Real F = 96485;
  Real R = 8.314;
  Real Tref = 298.15;
