...
registerMooseObject("corrosionApp", Nakkyu);
...
Nakkyu::validParams()
{
  ...
  params.addParam<Real>("kBB", 2.4444E-3, "Constant");
...
Nakkyu::Nakkyu(const InputParameters & parameters)
  : Material(parameters),
    _T(adCoupledValue("T")),
...
void
Nakkyu::initQpStatefulProperties()
{
  _ISS[_qp] = 0;
  _IEE[_qp] = 0;
  _Isum[_qp] = 0;
...
Nakkyu::computeQpProperties()
{
  ...
  //Calculate Corrosion Potential at given time and each quadruture point
          while (true)
                {
                  _Isum[_qp] = 
	           F * _nA * _Porosity * _kA * _C6[_qp] * _C6[_qp] * exp(F /(R * _T[_qp]) *(_Ecorr[_qp] - _EA))
	         + F *_nS * _Porosity * _kS * _C9[_qp] * _C9[_qp] * exp((1 + _aS) * F / (R * _T[_qp]) * _Ecorr[_qp]) * exp(-F / (R * _T[_qp]) * (_ES12 + _aS3 * _ES3)) 
		 - F * _nE * (1 - _Porosity) * _kE * _C9[_qp] * exp(-_aE * F / (R * _T[_qp]) * (_Ecorr[_qp] - _EE))
		 - F * _nF * (1 - _Porosity) * _kF * exp(-_aF * F / (R * _T[_qp]) * (_Ecorr[_qp] - _EF))
                 - F * _nA * _Porosity * _kBB * _C1[_qp];
 		  if (_Isum[_qp] < -_Tol)
  ...
  _IAA[_qp] = F * _nA * _Porosity * _kA * _C6[_qp] * _C6[_qp] * exp(F /(R * _T[_qp])*(_Ecorr[_qp] - _EA)) - F * _nA * _Porosity * _kBB * _C1[_qp];
  _ISS[_qp] = F *  _nS * _Porosity * _kS * _C9[_qp] * _C9[_qp] * exp((1 + _aS) * F / (R * _T[_qp]) * _Ecorr[_qp]) * exp(-F / (R * _T[_qp]) * (_ES12 + _aS3 * _ES3));
  _IEE[_qp] = -F * _nE * (1 - _Porosity) * _kE * _C9[_qp] * exp(-_aE * F / (R * _T[_qp]) * (_Ecorr[_qp] - _EE));
  _IFF[_qp] = -F * _nF * (1 - _Porosity) * _kF * exp(-_aF * F /(R * _T[_qp]) * (_Ecorr[_qp] - _EF));
}
