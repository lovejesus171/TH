...
#include "AnodicCurrent.h"

registerMooseObject("corrosionApp", AnodicCurrent);

template <>
InputParameters
validParams<AnodicCurrent>()
{
...
AnodicCurrent::AnodicCurrent(const InputParameters & parameters)
 ...
 */
Real
AnodicCurrent::computeValue()
{
  Real F = 96485;
  Real R = 8.314;

  return -(_nA * _Porosity * F * (_kA * _C6[_qp] * _C6[_qp] * exp( F / (R * _T[_qp]) * (_Ecorr[_qp] - _EA)) - _kB * _C1[_qp]) + _nS * _Porosity * F * _kS * _C9[_qp] * _C9[_qp] * exp((1+_aS) * F / (R * _T[_qp]) * _Ecorr[_qp]) * exp(-F / (R * _T[_qp] * (_ES12 + _aS3 * _ES3))));
}
