...
#include "ADAuxCu2SEVar.h"

registerMooseObject("corrosionApp", ADAuxCu2SEVar);

template <>
InputParameters
validParams<ADAuxCu2SEVar>()
{
  ...
  params.addCoupledVar("Corrosion_potential","Corrosion potential");
...
ADAuxCu2SEVar::ADAuxCu2SEVar(const InputParameters & parameters)
 ...
 */
Real
ADAuxCu2SEVar::computeValue()
{
return -MetaPhysicL::raw_value(_Num * _eps[_qp] * _kS * _C1[_qp] * _C1[_qp] * exp((1.0 + _aS) * _F /(_R * _T[_qp]) * _E[_qp]) * exp(-_F/(_R * _T[_qp]) * (_ES12 + _aS3 * _ES3)));
}
