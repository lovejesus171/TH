...
#include "CathodicCurrent.h"

registerMooseObject("corrosionApp", CathodicCurrent);

template <>
InputParameters
validParams<CathodicCurrent>()
{
...
CathodicCurrent::CathodicCurrent(const InputParameters & parameters)
 ...
 */
Real
CathodicCurrent::computeValue()
{
  Real F = 96485;
  Real R = 8.314;

  return _nD * (1 - _Porosity) * F * _kD * _C3[_qp] * exp(-_aD * F / (R * _T[_qp]) * (_Ecorr[_qp] - _ED))
        + _nE * (1 - _Porosity) * F * _kE * _C9[_qp] * exp(-_aE * F / (R * _T[_qp]) * (_Ecorr[_qp] - _EE))
        + _nF * (1 - _Porosity) * F * _kF * exp(-_aF * F / (R * _T[_qp]) * (_Ecorr[_qp] - _EF))
	  ;
    // Oxygen reduction 
