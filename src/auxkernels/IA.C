...
registerMooseObject("corrosionApp", IA);
...
IA::IA(const InputParameters & parameters)
...
IA::computeValue()
{
  ...
  return -(_nA * _Porosity * F * (_kA * _C6[_qp] * _C6[_qp] * exp( F / (R * _T[_qp]) * (_Ecorr[_qp] - _EA)) - _kB * _C1[_qp]));
}
