...
registerMooseObject("corrosionApp", MP5);
...
MP5::MP5(const InputParameters & parameters)
  : ADKernel(parameters),
...
ADReal
MP5::computeQpResidual()
{
  Real Alpha = 2 + _aS + _aE;
  Real E = _EA + _aE * _EE + _ES12 + _aS3 * _ES3;
  
  return _test[_i][_qp] * _u[_qp] - _test[_i][_qp] * ( E / Alpha + 8.314 * _T[_qp] / (96485 * Alpha) * log(_nE * (1 - _Porosity) * _kE * _kBB * _C1[_qp] / (_Porosity * _kA * _C6[_qp] * _C6[_qp] * _nS * _kS * _C9[_qp])));
