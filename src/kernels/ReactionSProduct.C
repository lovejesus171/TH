#include "ReactionSProduct.h"

// MOOSE includes
#include "MooseVariable.h"

registerMooseObject("corrosionApp", ReactionSProduct);

template <>
InputParameters
validParams<ReactionSProduct>()
{
...
ReactionSProduct::ReactionSProduct(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
ReactionSProduct::computeQpResidual()
{
  Real R = 8.314;
  Real T_Re = 298.15;

   if (_pp_value > 0 && _v[_qp] > 0)
...
Real
ReactionSProduct::computeQpJacobian()
{
  Real R = 8.314;
  Real T_Re = 298.15;
...
Real
ReactionSProduct::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real R = 8.314;
  Real T_Re = 298.15;
