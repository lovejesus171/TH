...
registerMooseObject("corrosionApp", Corr);
...
Corr::Corr(const InputParameters & parameters)
  : ADKernel(parameters),
...
Corr::computeQpResidual()
{
  ...
  return _test[_i][_qp]
	  * (
	   _nA * _Porosity * _kA * _C6[_qp] * _C6[_qp] * exp(F / (R * _T[_qp]) * (_u[_qp] - _EA))
	 + _nS * _Porosity * _kS * _C9[_qp] * _C9[_qp] * exp((1 + _aS) * F / (R * _T[_qp]) * _u[_qp]) * exp(-F / (R * _T[_qp] * (_ES12 + _aS3 * _ES3)))
	 - _nE * (1 - _Porosity) * _kE * _C9[_qp] * exp(-_aE * F / (R * _T[_qp]) * (_u[_qp] - _EE))
	 - _nF * (1 - _Porosity) * _kF * exp(-_aF * F / (R * _T[_qp]) * (_u[_qp] - _EF))
