...
#include "TwoPhaseTimeDerivative.h"
...
registerMooseObject("corrosionApp", TwoPhaseTimeDerivative);

InputParameters
TwoPhaseTimeDerivative::validParams()
{
...
TwoPhaseTimeDerivative::TwoPhaseTimeDerivative(const InputParameters & parameters)
...
Real
TwoPhaseTimeDerivative::computeQpResidual()
{
  // H = Henry's law constant
//  Real _H = 31.4;

  return _porosity[_qp] * _test[_i][_qp] * ((_fluid_saturation_nodal[_i][_ph] + (1 - _fluid_saturation_nodal[_i][_ph]) * 31.4) * _u[_qp] - (_fluid_saturation_nodal_old[_i][_ph] + (1 - _fluid_saturation_nodal_old[_i][_ph]) * 31.4) * _u_old[_qp]) / _dt;
}

Real
TwoPhaseTimeDerivative::computeQpJacobian()
{
//  Real _H = 31.4;
  // If the variable is not a PorousFlow variable (very unusual), the diag Jacobian terms are 0
  return _porosity[_qp] * _test[_i][_qp] * ((_fluid_saturation_nodal[_i][_ph] + (1 - _fluid_saturation_nodal[_i][_ph]) * 31.4) * _phi[_j][_qp]) / _dt;
}
