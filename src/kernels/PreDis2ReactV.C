...
registerMooseObject("corrosionApp", PreDis2ReactV);

template <>
InputParameters
validParams<PreDis2ReactV>()
{
...
PreDis2ReactV::PreDis2ReactV(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
PreDis2ReactV::computeQpResidual()
{
  Real R = 8.314;
  Real T_Re = 298.15;
     if(_v[_qp] <= _Cs[_qp])
       ...
       return -_test[_i][_qp] * _Num * _u[_qp] * (_v[_qp] - _Cs[_qp]) * _Reaction_rate * exp(_Ea / R * (1/T_Re - 1/_T[_qp])) ;
}

Real
PreDis2ReactV::computeQpJacobian()
{
  Real R = 8.314;
  Real T_Re = 298.15;
     if(_v[_qp] <= _Cs[_qp])
	...
	return -_test[_i][_qp] * _Num * _phi[_j][_qp] * (_v[_qp] - _Cs[_qp]) * _Reaction_rate * exp(_Ea / R * (1/T_Re - 1/_T[_qp])) ;
}

Real
PreDis2ReactV::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real R = 8.314;
  Real T_Re = 298.15;
	...
	return -_test[_i][_qp] * _Num * _u[_qp] * (_v[_qp] - _Cs[_qp]) * _Reaction_rate * _Ea / (R * _T[_qp] * _T[_qp]) * _phi[_j][_qp] * exp(_Ea / R * (1/T_Re - 1/_T[_qp]));
     else 
	return 0.0;
}
