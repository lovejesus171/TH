...
registerMooseObject("corrosionApp", MP3);
...
MP3::MP3(const InputParameters & parameters)
  : ADKernel(parameters),
...
ADReal
MP3::computeQpResidual()
{
  Real Alpha = 1 + _aS + _aE;
  Real E = _ES12 + _aS3 * _ES3 + _aE * _EE;
  
  return _test[_i][_qp] * _u[_qp] - _test[_i][_qp] * ( E / Alpha + 8.314 * 298.15 / (96485 * Alpha) * log(_nE * _kE * (1 - _Porosity + _Area)/ (_Porosity *_nS * _kS * _C9[_qp])));
