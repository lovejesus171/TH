...
registerMooseObject("corrosionApp", MP2);
...
MP2::MP2(const InputParameters & parameters)
  : ADKernel(parameters),
...
ADReal
MP2::computeQpResidual()
{
  Real Alpha = 2 + _aS + _aE;
  Real E = _EA + _ES12 + _aS3 * _ES3 + _aE * _EE;
  
  return _test[_i][_qp] * _u[_qp] - _test[_i][_qp] * ( E / Alpha + 8.314 * 298.15 / (96485 * Alpha) * log(_kBB * _C1[_qp] * _nE * _kE * (1 - _Porosity + _Area)/ (_nA * _kA * _Porosity * _Porosity * _C6[_qp] * _C6[_qp] * _nS * _kS * _C9[_qp])));
