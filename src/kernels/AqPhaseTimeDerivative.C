...
#include "AqPhaseTimeDerivative.h"
...
registerMooseObject("corrosionApp", AqPhaseTimeDerivative);

InputParameters
AqPhaseTimeDerivative::validParams()
{
...
AqPhaseTimeDerivative::AqPhaseTimeDerivative(const InputParameters & parameters)
...
Real
AqPhaseTimeDerivative::computeQpResidual()
{
  // H = Henry's law constant
//  Real _H = 31.4;

  return _porosity[_qp] * _test[_i][_qp] * (_fluid_saturation_nodal[_i][_ph] * _u[_qp] - _fluid_saturation_nodal_old[_i][_ph] * _u_old[_qp]) / _dt;
}

Real
AqPhaseTimeDerivative::computeQpJacobian()
{
//  Real _H = 31.4;
  // If the variable is not a PorousFlow variable (very unusual), the diag Jacobian terms are 0
  return _porosity[_qp] * _test[_i][_qp] * (_fluid_saturation_nodal[_i][_ph] * _phi[_j][_qp]) / _dt;
}
